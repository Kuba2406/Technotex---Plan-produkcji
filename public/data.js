...
function buildArticleHistory() {
  const history = {};
  ARTYKULY_INIT.forEach((art, index) => {
    const createdDay = String(2 + index).padStart(2, '0');
    const updatedDay = String(10 + index).padStart(2, '0');
    history[art.id] = [
      {
        data: `2026-06-${createdDay}`,
        typ: 'utworzenie',
        opis: `Dodano artykuł ${art.nazwa} do kartoteki technologicznej.`,
        uzytkownik: 'Planista',
      },
      {
        data: `2026-06-${updatedDay}`,
        typ: 'aktualizacja',
        opis: art.uwagi
          ? `Uzupełniono parametry i uwagi technologiczne (${art.szerokoscTkaniny || '—'} cm).`
          : `Zweryfikowano parametry technologiczne i szerokość tkaniny (${art.szerokoscTkaniny || '—'} cm).`,
        uzytkownik: 'Technolog',
      },
    ];
  });
  return history;
...
const HISTORIA_ARTYKULOW_INIT = buildArticleHistory();
...
// ---- Application state (mutated at runtime, persisted via localStorage + API) ----
const state = {
  ...
  historiaArtykulow: JSON.parse(JSON.stringify(HISTORIA_ARTYKULOW_INIT)),
...
function addArtykulHistory(artId, typ, opis, uzytkownik) {
  if (!state.historiaArtykulow[artId]) state.historiaArtykulow[artId] = [];
  state.historiaArtykulow[artId].unshift({
    data: new Date().toISOString().split('T')[0],
    typ,
    opis,
    uzytkownik: uzytkownik || 'Planista',
  });
}
...
// ============================================================
// Persistence layer – localStorage + backend API
// ============================================================

const STATE_STORAGE_KEY = 'technotex_state_v1';
const STATE_KEYS_TO_PERSIST = [
  'artykuly', 'typyKrosien', 'krosna', 'osnowy', 'pracownicy',
  'zmianyTygodniowe', 'nieobecnosci', 'zlecenia', 'snowalnia',
  'klejarnia', 'historiaArtykulow', 'historiaKrosien', 'obecnosci', 'nextId',
];

/** Extract only the persistable slice of state. */
function getPersistedSlice() {
  const slice = {};
  STATE_KEYS_TO_PERSIST.forEach(function(k) { slice[k] = state[k]; });
  return slice;
}

/** Merge a persisted snapshot back into the live state object. */
function applySnapshot(snapshot) {
  if (!snapshot || typeof snapshot !== 'object') return;
  STATE_KEYS_TO_PERSIST.forEach(function(k) {
...
function saveToLocalStorage() {
  try {
    localStorage.setItem(STATE_STORAGE_KEY, JSON.stringify(getPersistedSlice()));
  } catch (e) {
...
let _apiSavePending = false;
let _apiSaveTimeout = null;

function scheduleApiSave() {
  if (_apiSaveTimeout) clearTimeout(_apiSaveTimeout);
  _apiSaveTimeout = setTimeout(function() { persistToApi(); }, 3000);
}

function persistToApi() {
  if (_apiSavePending) return;
  _apiSavePending = true;
  fetch('/api/state', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(getPersistedSlice()),
  })
    .then(function(r) { return r.ok ? null : Promise.reject(r.status); })
    .catch(function() { /* Backend not available — silently ignore */ })
    .finally(function() { _apiSavePending = false; });
}

/** Call after any state mutation that should be persisted. */
...
(function initPersistence() {
  // 1. Try localStorage first (instant, no network needed)
  var local = loadFromLocalStorage();
  if (local) {
    applySnapshot(local);
  }

  // 2. Then try backend API (may override local if backend has newer data)
  fetch('/api/state')
    .then(function(r) { return r.ok ? r.json() : null; })
    .then(function(body) {
      if (body && body.data) {
        applySnapshot(body.data);
        saveToLocalStorage(); // sync localStorage with backend
        // Trigger re-render if app.js is already loaded
        if (typeof renderView === 'function') renderView();
      }
    })
    .catch(function() { /* Backend not available — use local/seed data */ });
})();

// ---- Auto-save interval (localStorage every 5 seconds) ----
